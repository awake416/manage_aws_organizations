locals {
  ...
  all_accounts = [for name, item in var.accounts : item.id]
  prod_accounts = [for name, item in var.accounts : item.id if (length(regexall(".*prod", name)) > 0)]
  non_prod_accounts = setsubtract(setsubtract(local.all_accounts, local.prod_accounts), local.dev_accounts)
